...
use crate::{
    account::{Account, AccountType, Entry, Side, Source, Transaction, TransactionStatus},
...
impl InterestTerms {
    fn clamped_day_for_month(year: i32, month: u32, day: i32) -> u32 {
        ...
        (day.max(1) as u32).min(last_day_of_month)
    }

    fn payment_date_after_period_end(&self, period_end_date: NaiveDate) -> NaiveDate {
        let mut year = period_end_date.year();
        let mut month = period_end_date.month();

        let day = Self::clamped_day_for_month(year, month, self.paid_day);
        let mut payment_date = NaiveDate::from_ymd_opt(year, month, day).unwrap();

        if payment_date < period_end_date {
            if month == 12 {
                year += 1;
                month = 1;
            } else {
            ...
            let next_month_day = Self::clamped_day_for_month(year, month, self.paid_day);
            payment_date = NaiveDate::from_ymd_opt(year, month, next_month_day).unwrap();
        }
    ...
    pub fn from_components(
        ...
        description: String,
        income_account_id: Option<Uuid>,
        interest_account_id: Option<Uuid>,
    ) -> Self {
        InterestTerms {
            id: Uuid::new_v4(),
            start_date,
...
impl Interest {
    ...
    pub fn get_terms_for_date(&self, date: NaiveDate) -> Vec<&InterestTerms> {
        self.terms
            .iter()
            .filter(|t| t.start_date <= date && t.end_date.map_or(true, |end| end >= date))
            .collect()
    }
    ...
    pub fn get_start_date(&self) -> Option<&NaiveDate> {
        self.terms
            .iter()
            .min_by(|t1, t2| t1.start_date.cmp(&t2.start_date))
            .map(|t| &t.start_date)
    }
}
...
fn build_interest_transaction(
    ...
    if is_interest_bearing || interest_account.as_ref().is_some() {
        transaction.entries.push(Entry {
            ...
            account_id: if is_interest_bearing {
                source_account.id
            } else {
                interest_account.as_ref().unwrap().id
            },
            date: cur_date,
...
pub fn calculate_interest_for_accounts(
    books: &mut Books,
    interest_accounts: Vec<Account>,
    to_date: NaiveDate,
) -> Result<(), BooksError> {
    ...
    for account in interest_accounts {
        let Some(interest_id) = account.interest_id else {
            continue;
        };
        let Ok(interest) = books.get_interest(&interest_id) else {
            continue;
        };
        ...
        let existing_interest_transactions =
            books.transactions_by_interest(interest.id, None, None);
        ...
        let last_recorded_before_earliest = match earliest_projected_date {
            Some(projected_date) => recorded_dates
                .iter()
                .copied()
                .filter(|d| *d < projected_date)
                .max(),
    ...
    let Some(earliest_start_date) = earliest_start_date else {
        return Ok(());
    };
    ...
    for state in &mut states {
        ...
        while state.entry_index < state.entries.len()
            && state.entries[state.entry_index].date < earliest_start_date
        {
    ...
    while cur_date <= to_date {
        for state in &mut states {
            ...
            while state.entry_index < state.entries.len()
                && state.entries[state.entry_index].date <= cur_date
            {
                let entry = &state.entries[state.entry_index];
                if entry.entry_type == normal_balance {
                    state.balance += entry.amount;
            ...
            for terms in cur_terms {
                ...
                if state.balance >= min_balance {
                    let term_interest_paid = state
                        .interest_paid_by_term
                        .entry(terms.id)
                        .or_insert(dec!(0));
                    let interest_amount: Decimal;
                    if terms.max_balance.is_some()
                        && (state.balance + *term_interest_paid) >= terms.max_balance.unwrap()
                    {
                        interest_amount = daily_rate * (terms.max_balance.unwrap() - min_balance);
                    } else {
                        interest_amount =
                            daily_rate * (state.balance + *term_interest_paid - min_balance);
                    }

                    let current_balance = state
                        .interest_tally_by_term
                        .entry(terms.id)
                        .or_insert(dec!(0));
...
fn add_transaction_and_record_deltas(
    ...
    pending_deltas: &mut HashMap<Uuid, HashMap<NaiveDate, Decimal>>,
    delta_effective_date: NaiveDate,
) {
...
fn settle_interest_periods(
    ...
    let mut payouts_by_account: HashMap<(NaiveDate, Option<Uuid>, Option<Uuid>, String), Decimal> =
        HashMap::new();
    let mut cleared_term_ids: Vec<Uuid> = Vec::new();

    for term_id in term_ids {
        let Some(term) = state.term_by_id.get(&term_id) else {
            continue;
        };
        if term.is_end_of_interest_period(period_end_date) {
            let balance = state
                ...
                .unwrap_or(dec!(0));
            if balance > dec!(0) {
                let payment_date = term.payment_date_after_period_end(period_end_date);
                payouts_by_account
                    .entry((
                        payment_date,
                        term.income_account_id,
                        term.interest_account_id,
                        term.description.clone(),
                    ))
    ...
    if !payouts_by_account.is_empty() {
        for ((payment_date, income_account_id, interest_account_id, description), balance) in payouts_by_account {
            ...
            if !state.recorded_transaction_keys.contains(&transaction_key) {
                ...
                let posting_account = match interest_account_id {
                    Some(account_id) => match books.get_account(&account_id) {
                        ...
                        Err(e) => {
                            println!(
                                "Error loading interest posting account {}: {:?}",
                                account_id, e
                            );
                            continue;
                        }
                ...
                let delta_effective_date = if payment_date <= period_end_date {
                    period_end_date.succ_opt().unwrap()
                } else {
                    payment_date
                };
                add_transaction_and_record_deltas(
                    books,
                    tx,
                    state_index_by_account,
                    pending_deltas,
                    delta_effective_date,
                );
            }
        }
    }
...
mod tests {
    ...
    use crate::{
        account::{Account, AccountType, Entry, Side, Source, Transaction, TransactionStatus},
        books::Books,
        interest::{calculate_interest_for_accounts, Interest, InterestTerms, InterestType},
        schedule::ScheduleEnum,
    };
    ...
    fn calculate_loan_interest_daily() {
        ...
        let mut transaction_account =
            Account::create_new("Transaction Account 1", AccountType::Asset);
        ...
        let _ = books.add_transaction(build_transaction(
            Some(transaction_account.id),
            Some(loan_account.id),
        ...
        let transactions = books
            .transactions()
            .iter()
            .filter(|t| t.source_type == Some(Source::Interest))
            .collect::<Vec<_>>();
        ...
        assert_eq!(
            transactions[0].entries[0].date,
            NaiveDate::from_ymd_opt(2022, 2, 1).unwrap()
        );
        assert_eq!(
            transactions[0].entries[1].date,
            NaiveDate::from_ymd_opt(2022, 2, 1).unwrap()
        );
        ...
        assert_eq!(
            transactions[1].entries[0].date,
            NaiveDate::from_ymd_opt(2022, 3, 1).unwrap()
        );
        assert_eq!(
            transactions[1].entries[1].date,
            NaiveDate::from_ymd_opt(2022, 3, 1).unwrap()
        );
    }
    ...
    fn calculate_loan_interest_daily_on_starting_balance() {
        ...
        assert_eq!(
            transactions[11].entries[0].date,
            NaiveDate::from_ymd_opt(2023, 1, 1).unwrap()
        );
        assert_eq!(
            transactions[11].entries[1].date,
            NaiveDate::from_ymd_opt(2023, 1, 1).unwrap()
        );
    }
    ...
    fn calculate_interest_daily_on_starting_balance() {
        ...
        let interest =
            Interest::from_components(vec![interest_terms_1, interest_terms_2], savings_account.id);
        calculate_interest_wrapper(
            &mut books,
            interest,
            NaiveDate::from_ymd_opt(2022, 12, 31).unwrap(),
    ...
    fn calculate_interest_daily_on_starting_balance_paid_to_a_different_account() {
        ...
        assert_eq!(
            transactions[0].entries[0].description,
            "Interest paid elsewhere".to_string()
        );
        assert_eq!(
            transactions[0].entries[1].description,
            "Interest paid elsewhere".to_string()
        );
    }
    ...
    fn calculate_tiered_interest_daily() {
        ...
        let interest =
            Interest::from_components(vec![tier_1_terms, tier_2_terms], savings_account.id);
        calculate_interest_wrapper(
            &mut books,
            interest,
            NaiveDate::from_ymd_opt(2022, 2, 28).unwrap(),
    ...
    fn calculate_interest_recalculates_from_today() {
        ...
        let interest_transactions =
            books.transactions_by_interest(savings_account.interest_id.unwrap(), None, None);
        assert!(interest_transactions.iter().all(|t| t.id != existing_id));
        assert!(interest_transactions
            .iter()
            .any(|t| t.date() == end_of_month.succ_opt()));
    }
    ...
    fn calculate_interest_preserves_recorded_transactions() {
        ...
        assert!(interest_transactions
            .iter()
            .filter(|t| t.date() == Some(payment_date))
            .all(|t| t.status == TransactionStatus::Recorded));
    }
    ...
    fn calculate_interest_multi_account_different_terms() {
        ...
        calculate_interest_for_accounts(
            &mut books,
            interest_accounts,
        ...
        let account_1_transactions: Vec<_> = transactions
            .iter()
            .filter(|t| {
                t.entries
                    .iter()
                    .any(|e| e.account_id == interest_earned_1.id)
            })
            .collect();
        let account_2_transactions: Vec<_> = transactions
            .iter()
            .filter(|t| {
                t.entries
                    .iter()
                    .any(|e| e.account_id == interest_earned_2.id)
            })
            .collect();
        ...
        let jan_account_1 = account_1_transactions
            .iter()
        ...
        let jan_account_2 = account_2_transactions
            .iter()
            .find(|t| t.date() == Some(NaiveDate::from_ymd_opt(2022, 2, 1).unwrap()))
            .unwrap();
    ...
    fn calculate_interest_with_transfer() {
        ...
        let jan_transactions: Vec<_> = transactions
            .iter()
            .filter(|t| t.date() == Some(NaiveDate::from_ymd_opt(2022, 2, 1).unwrap()))
            .collect();
        let feb_transactions: Vec<_> = transactions
            .iter()
        ...
        let jan_account_1_to_2 = jan_transactions
            .iter()
            .find(|t| {
                t.entries
                    .iter()
            ...
            .unwrap();
        let jan_account_2_to_1 = jan_transactions
            .iter()
            .find(|t| {
                t.entries
                    .iter()
        ...
        let feb_account_1 = feb_transactions
            .iter()
            .find(|t| {
                t.entries
                    .iter()
                    .any(|e| e.account_id == savings_account_1.id)
            })
            .unwrap();
        let feb_account_2 = feb_transactions
            .iter()
            .find(|t| {
                t.entries
                    .iter()
                    .any(|e| e.account_id == savings_account_2.id)
            })
            .unwrap();
    ...
    fn test_is_end_of_month_february_non_leap() {
        ...
        // February 2022 has 28 days, so paid_day 31 is clamped to 28 and
        // period end is the previous day (27th).
        ...
        assert!(!terms.is_end_of_interest_period(NaiveDate::from_ymd_opt(2022, 2, 26).unwrap()));
        assert!(!terms.is_end_of_interest_period(NaiveDate::from_ymd_opt(2022, 2, 1).unwrap()));
    }

    #[test]
    fn test_is_end_of_month_short_month_pays_last_valid_day() {
        let terms = InterestTerms::simple(
            NaiveDate::from_ymd_opt(2022, 1, 1).unwrap(),
            dec!(0.05),
            InterestType::Daily,
        ...
        // September has 30 days, so payment should be on the 30th and
        // period end should be on the 29th.
        assert!(terms.is_end_of_interest_period(NaiveDate::from_ymd_opt(2022, 9, 29).unwrap()));
        assert!(!terms.is_end_of_interest_period(NaiveDate::from_ymd_opt(2022, 9, 30).unwrap()));
        assert!(!terms.is_end_of_interest_period(NaiveDate::from_ymd_opt(2022, 9, 28).unwrap()));
    }
    ...
    fn test_is_end_of_month_february_leap() {
        ...
        // February 2020 has 29 days, so paid_day 31 is clamped to 29 and
        // period end is the previous day (28th).
    ...
    #[test]
    fn calculate_interest_uses_paid_day_for_payment_date() {
        let mut books = Books::build_empty("My Books");
        let mut savings_account = Account::create_new("Savings Account 1", AccountType::Asset);
        savings_account.starting_balance = dec!(10000);
        books.add_account(savings_account.clone());
        ...
        let mut interest_terms = InterestTerms::simple(
            NaiveDate::from_ymd_opt(2022, 1, 1).unwrap(),
            dec!(0.05),
            InterestType::Daily,
            ScheduleEnum::Months,
        ...
        interest_terms.period_start_day = 15;
    ...
    #[test]
    fn calculate_interest_paid_day_changes_payment_date_not_first_period_amount() {
        let mut books_early = Books::build_empty("My Books");
        let mut books_late = Books::build_empty("My Books");

        let mut savings_early = Account::create_new("Savings Account 1", AccountType::Asset);
        savings_early.starting_balance = dec!(10000);
        let mut savings_late = Account::create_new("Savings Account 1", AccountType::Asset);
        savings_late.starting_balance = dec!(10000);

        books_early.add_account(savings_early.clone());
        books_late.add_account(savings_late.clone());

        let interest_early = Account::create_new("Interest Earned", AccountType::Revenue);
        let interest_late = Account::create_new("Interest Earned", AccountType::Revenue);
        books_early.add_account(interest_early.clone());
        books_late.add_account(interest_late.clone());

        let mut terms_early = InterestTerms::simple(
            NaiveDate::from_ymd_opt(2022, 1, 1).unwrap(),
            dec!(0.05),
            InterestType::Daily,
            ScheduleEnum::Months,
            ...
            16,
            "Interest payment".to_string(),
            Some(interest_early.id),
        );
        terms_early.period_start_day = 15;

        let mut terms_late = InterestTerms::simple(
            NaiveDate::from_ymd_opt(2022, 1, 1).unwrap(),
            dec!(0.05),
            InterestType::Daily,
            ScheduleEnum::Months,
            ...
            28,
            "Interest payment".to_string(),
            Some(interest_late.id),
        );
        terms_late.period_start_day = 15;

        let interest1 = Interest::from_components(vec![terms_early], savings_early.id);
        let interest2 = Interest::from_components(vec![terms_late], savings_late.id);
        ...
        let tx_early = books_early
            .transactions()
            .iter()
            .filter(|t| t.source_type == Some(Source::Interest))
            .collect::<Vec<_>>();
        let tx_late = books_late
            .transactions()
            .iter()
            .filter(|t| t.source_type == Some(Source::Interest))
            .collect::<Vec<_>>();

        assert_eq!(tx_early.len(), 1);
        assert_eq!(tx_late.len(), 1);

        assert_eq!(
            tx_early[0].entries[0].date,
            NaiveDate::from_ymd_opt(2022, 1, 16).unwrap()
        );
        assert_eq!(
            tx_late[0].entries[0].date,
            NaiveDate::from_ymd_opt(2022, 1, 28).unwrap()
        );

        assert_eq!(tx_early[0].entries[0].amount, tx_late[0].entries[0].amount);
        assert_eq!(tx_early[0].entries[1].amount, tx_late[0].entries[1].amount);
    }

    #[test]
    fn calculate_interest_paid_day_on_period_end_posts_in_same_period() {
        let mut books = Books::build_empty("My Books");
        let mut savings_account = Account::create_new("Savings Account 1", AccountType::Asset);
        savings_account.starting_balance = dec!(10000);
        books.add_account(savings_account.clone());
    ...
    #[test]
    fn calculate_interest_payment_on_period_end_affects_next_period_balance() {
        let mut books = Books::build_empty("My Books");
        let mut savings_account = Account::create_new("Savings Account 1", AccountType::Asset);
        savings_account.starting_balance = dec!(10000);
        books.add_account(savings_account.clone());
        ...
        // Monthly period: 1st..end-of-month, paid on the end-of-month day.
        let terms = InterestTerms::from_components(
            NaiveDate::from_ymd_opt(2022, 1, 1).unwrap(),
            None,
            dec!(0.05),
            InterestType::Daily,
        ...
        let interest = Interest::from_components(vec![terms], savings_account.id);
        calculate_interest_wrapper(
            &mut books,
            interest,
            NaiveDate::from_ymd_opt(2022, 2, 28).unwrap(),
        ...
        assert_eq!(transactions.len(), 2);
        assert_eq!(transactions[0].entries[0].amount, dec!(42.47));
        // If Jan payment is not carried into Feb balance, this is 38.36.
        assert_eq!(transactions[1].entries[0].amount, dec!(38.52));
    }
    ...
    fn test_is_end_of_month_various_paid_days() {
        ...
        let test_cases = vec![
            ...
            (31, 30), // paid_day 31 -> paid_to_day 30
            (32, 30), // paid_day clamps to month end (31) -> paid_to_day 30
        ];
        ...
        for (paid_day, expected_paid_to_day) in test_cases {
            ...
            assert!(
                terms.is_end_of_interest_period(
                    NaiveDate::from_ymd_opt(2022, 1, expected_paid_to_day).unwrap()
                ),
                "Failed for paid_day {} on day {}",
                paid_day,
                expected_paid_to_day
            );
            ...
            if expected_paid_to_day > 1 {
                assert!(
                    !terms.is_end_of_interest_period(
                        NaiveDate::from_ymd_opt(2022, 1, expected_paid_to_day - 1).unwrap()
                    ),
                    "Should not trigger on day {} for paid_day {}",
                    expected_paid_to_day - 1,
                    paid_day
                );
            }
